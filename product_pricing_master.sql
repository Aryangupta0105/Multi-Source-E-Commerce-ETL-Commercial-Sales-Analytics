...
LIMIT 5;
SELECT * FROM `May-2022` LIMIT 5;
UPDATE `MAY-2022`
SET
   `Sku`       = upper(TRIM(`SKU`)),
   `Style ID`  = upper(TRIM(`Style ID`)),
   `catalog`   = upper(TRIM(`Catalog`)),
   `category`  = upper(TRIM(`category`));
UPDATE `May-2022`
SET 
    `Weight`         = CASE WHEN `Weight` IN ('Nill', '', '#VALUE!') OR `Weight` IS NULL THEN 0 ELSE `Weight` END,
    `TP`             = CASE WHEN `TP` IN ('Nill', '', '#VALUE!') OR `TP` IS NULL THEN 0 ELSE `TP` END,
    `MRP Old`        = CASE WHEN `MRP Old` IN ('Nill', '', '#VALUE!') OR `MRP Old` IS NULL THEN 0 ELSE `MRP Old` END,
    `Final MRP Old`  = CASE WHEN `Final MRP Old` IN ('Nill', '', '#VALUE!') OR `Final MRP Old` IS NULL THEN 0 ELSE `Final MRP Old` END,
    `Ajio MRP`       = CASE WHEN `Ajio MRP` IN ('Nill', '', '#VALUE!') OR `Ajio MRP` IS NULL THEN 0 ELSE `Ajio MRP` END,
    `Amazon MRP`     = CASE WHEN `Amazon MRP` IN ('Nill', '', '#VALUE!') OR `Amazon MRP` IS NULL THEN 0 ELSE `Amazon MRP` END,
    `Amazon FBA MRP` = CASE WHEN `Amazon FBA MRP` IN ('Nill', '', '#VALUE!') OR `Amazon FBA MRP` IS NULL THEN 0 ELSE `Amazon FBA MRP` END,
    `Flipkart MRP`   = CASE WHEN `Flipkart MRP` IN ('Nill', '', '#VALUE!') OR `Flipkart MRP` IS NULL THEN 0 ELSE `Flipkart MRP` END,
    `Limeroad MRP`   = CASE WHEN `Limeroad MRP` IN ('Nill', '', '#VALUE!') OR `Limeroad MRP` IS NULL THEN 0 ELSE `Limeroad MRP` END,
    `Myntra MRP`     = CASE WHEN `Myntra MRP` IN ('Nill', '', '#VALUE!') OR `Myntra MRP` IS NULL THEN 0 ELSE `Myntra MRP` END,
    `Paytm MRP`      = CASE WHEN `Paytm MRP` IN ('Nill', '', '#VALUE!') OR `Paytm MRP` IS NULL THEN 0 ELSE `Paytm MRP` END,
    `Snapdeal MRP`   = CASE WHEN `Snapdeal MRP` IN ('Nill', '', '#VALUE!') OR `Snapdeal MRP` IS NULL THEN 0 ELSE `Snapdeal MRP` END;
    ALTER TABLE `May-2022` 
    MODIFY COLUMN `Weight` VARCHAR(50),
    MODIFY COLUMN `TP` VARCHAR(50),
    MODIFY COLUMN `MRP Old` VARCHAR(50),
    MODIFY COLUMN `Final MRP Old` VARCHAR(50),
    MODIFY COLUMN `Ajio MRP` VARCHAR(50),
    MODIFY COLUMN `Amazon MRP` VARCHAR(50),
    MODIFY COLUMN `Amazon FBA MRP` VARCHAR(50),
    MODIFY COLUMN `Flipkart MRP` VARCHAR(50),
    MODIFY COLUMN `Limeroad MRP` VARCHAR(50),
    MODIFY COLUMN `Myntra MRP` VARCHAR(50),
    MODIFY COLUMN `Paytm MRP` VARCHAR(50),
    MODIFY COLUMN `Snapdeal MRP` VARCHAR(50);
    UPDATE `May-2022`
SET 
    `Weight`         = CASE WHEN TRIM(`Weight`) IN ('Nill', '#VALUE!', '') OR `Weight` IS NULL THEN '0' ELSE TRIM(`Weight`) END,
    `TP`             = CASE WHEN TRIM(`TP`) IN ('Nill', '#VALUE!', '') OR `TP` IS NULL THEN '0' ELSE TRIM(`TP`) END,
    `MRP Old`        = CASE WHEN TRIM(`MRP Old`) IN ('Nill', '#VALUE!', '') OR `MRP Old` IS NULL THEN '0' ELSE TRIM(`MRP Old`) END,
    `Final MRP Old`  = CASE WHEN TRIM(`Final MRP Old`) IN ('Nill', '#VALUE!', '') OR `Final MRP Old` IS NULL THEN '0' ELSE TRIM(`Final MRP Old`) END,
    `Ajio MRP`       = CASE WHEN TRIM(`Ajio MRP`) IN ('Nill', '#VALUE!', '') OR `Ajio MRP` IS NULL THEN '0' ELSE TRIM(`Ajio MRP`) END,
    `Amazon MRP`     = CASE WHEN TRIM(`Amazon MRP`) IN ('Nill', '#VALUE!', '') OR `Amazon MRP` IS NULL THEN '0' ELSE TRIM(`Amazon MRP`) END,
    `Amazon FBA MRP` = CASE WHEN TRIM(`Amazon FBA MRP`) IN ('Nill', '#VALUE!', '') OR `Amazon FBA MRP` IS NULL THEN '0' ELSE TRIM(`Amazon FBA MRP`) END,
    `Flipkart MRP`   = CASE WHEN TRIM(`Flipkart MRP`) IN ('Nill', '#VALUE!', '') OR `Flipkart MRP` IS NULL THEN '0' ELSE TRIM(`Flipkart MRP`) END,
    `Limeroad MRP`   = CASE WHEN TRIM(`Limeroad MRP`) IN ('Nill', '#VALUE!', '') OR `Limeroad MRP` IS NULL THEN '0' ELSE TRIM(`Limeroad MRP`) END,
    `Myntra MRP`     = CASE WHEN TRIM(`Myntra MRP`) IN ('Nill', '#VALUE!', '') OR `Myntra MRP` IS NULL THEN '0' ELSE TRIM(`Myntra MRP`) END,
    `Paytm MRP`      = CASE WHEN TRIM(`Paytm MRP`) IN ('Nill', '#VALUE!', '') OR `Paytm MRP` IS NULL THEN '0' ELSE TRIM(`Paytm MRP`) END,
    `Snapdeal MRP`   = CASE WHEN TRIM(`Snapdeal MRP`) IN ('Nill', '#VALUE!', '') OR `Snapdeal MRP` IS NULL THEN '0' ELSE TRIM(`Snapdeal MRP`) END;
    UPDATE `May-2022`
SET 
    `Sku`      = UPPER(TRIM(`Sku`)),
    `Style Id` = UPPER(TRIM(`Style Id`)),
    ...
    MODIFY COLUMN `TP` DECIMAL(10, 2),
    MODIFY COLUMN `MRP Old` DECIMAL(10, 2),
    MODIFY COLUMN `Final MRP Old` DECIMAL(10, 2),
    MODIFY COLUMN `Ajio MRP` DECIMAL(10, 2),
    MODIFY COLUMN `Amazon MRP` DECIMAL(10, 2),
    MODIFY COLUMN `Amazon FBA MRP` DECIMAL(10, 2),
    MODIFY COLUMN `Flipkart MRP` DECIMAL(10, 2),
    MODIFY COLUMN `Limeroad MRP` DECIMAL(10, 2),
    MODIFY COLUMN `Myntra MRP` DECIMAL(10, 2),
    MODIFY COLUMN `Paytm MRP` DECIMAL(10, 2),
    MODIFY COLUMN `Snapdeal MRP` DECIMAL(10, 2);
    RENAME TABLE `May-2022` TO product_pricing_master;
    SELECT `Sku`, `Category`, `Weight`, `TP`, `Amazon MRP`, `Flipkart MRP` 
FROM product_pricing_master 
LIMIT 10;
-- Compare Transfer Price (TP) vs Average Selling Price across channels
SELECT 
    `Category`,
    COUNT(*) AS total_skus,
    ROUND(AVG(`TP`), 2) AS avg_transfer_price,
    ROUND(AVG(`Amazon MRP`), 2) AS avg_amazon_price,
    ROUND(AVG(`Myntra MRP`), 2) AS avg_myntra_price,
    ROUND(AVG(`Flipkart MRP`), 2) AS avg_flipkart_price,
    ROUND(AVG(`Amazon MRP` - `TP`), 2) AS avg_gross_margin_amazon
FROM product_pricing_master
WHERE `TP` > 0
GROUP BY `Category`
